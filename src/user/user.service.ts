...
/*    if(isSameUser && id === isSameUser.id) throw new BadRequestException(
  ...
  async subscribe(id:number, channelId:number){
    ...
    const isSubscribed = await this.subscriptionRepository.findOneBy(UserAndChannel)

    if(!isSubscribed){
      const newSubscription = await this.subscriptionRepository.create(UserAndChannel)
